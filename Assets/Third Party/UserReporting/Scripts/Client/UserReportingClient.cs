	...
	public class UserReportingClient
	{
		...
		private void LogEvent(UserReportEventLevel level, string message, string stackTrace, Exception exception)
		{
			lock (events)
			{
				...
				{
					Level = level,
					Message = GetCleanText(regex, message),
					FrameNumber = frameNumber,
					StackTrace = GetCleanText(regex, stackTrace),
		...
		private string GetCleanText(Regex regex, string text)
		{
			...
			Action<bool, UserReport> callback)
		{
			try
			{
				...
				{
					RRLogger.Error("Identifier cannot be set on the client side. The value provided was discarded.");
					return;
				}

				if (userReport.ContentLength != 0)
				{
					RRLogger.Error("ContentLength cannot be set on the client side. The value provided was discarded.");
					return;
				}

				if (userReport.ReceivedOn != default)
				{
					RRLogger.Error("ReceivedOn cannot be set on the client side. The value provided was discarded.");
					return;
				}

				if (userReport.ExpiresOn != default)
				{
					RRLogger.Error("ExpiresOn cannot be set on the client side. The value provided was discarded.");
					return;
				}

				RRLogger.Log("Sending user report.");
				var json = Platform.SerializeJson(userReport);
				var jsonData = Encoding.UTF8.GetBytes(json);
				var endpoint = GetEndpoint();
				var url = string.Format($"{endpoint}/api/userreporting");
				Platform.Post(url, "application/json", jsonData,
					(uploadProgress,
						downloadProgress) => progressCallback?.Invoke(uploadProgress, downloadProgress),
					(success, result) =>
					{
							...
							if (success)
							{
								try
								{
									...
									if (userReportResult != null)
									{
										if (SendEventsToAnalytics)
										{
											...
											{
												{ "UserReportIdentifier", userReport.Identifier }
											};
											Platform.SendAnalyticsEvent("UserReportingClient.SendUserReport",
												eventData);
										}

										callback(true, userReportResult);
									}
									else
									{
										callback(false, null);
								...
								catch (Exception ex)
								{
									...
									LogEvent(UserReportEventLevel.Error,
										$"Sending user report failed: {ex}");
									callback(false, null);
								}
							}
							else
							{
								RRLogger.Error("Sending user report failed.");
		...
		public void TakeScreenshot(int maximumWidth, int maximumHeight, Action<UserReportScreenshot> callback = null)
		{
		...
		public void TakeScreenshotFromSource(int maximumWidth, int maximumHeight, object source,
			Action<UserReportScreenshot> callback = null)
		{
			...
			Platform.TakeScreenshot(frameNumber, maximumWidth, maximumHeight, source, (passedFrameNumber, data) =>
			{
				AddSynchronizedAction(() =>
				{
					lock (screenshots)
					{
						...
						screenshotsSaved++;
						callback?.Invoke(userReportScreenshot);
					}
				});
			});
		}
