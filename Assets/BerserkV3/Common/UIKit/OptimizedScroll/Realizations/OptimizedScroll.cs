...
{
	public class OptimizedScroll : OSA<BaseParamsWithPrefab, WidgetViewsHolder>
	{
		// Helper that stores data and notifies the adapter when items count changes
		// Can be iterated and can also have its elements accessed by the [] operator
		public SimpleDataHelper<WidgetScrollItemModel> Data { get; private set; }
		
		private List<WidgetScrollItemModel> models = new List<WidgetScrollItemModel>();

		private bool isInitialized;

		public bool IsInitialized => isInitialized;

		public void UpdateContent(List<ScrollWidgetModel> contexts)
		{
			if(!isInitialized)
			{
				InitScroll(contexts);
			}
			else if(contexts.Count == models.Count)
			{
				Data.ResetItems(models);
			}
			else if (contexts.Count != models.Count)
			{
				RetrieveDataAndUpdate(contexts);
			}
		}

		public void ForceUpdateContent(List<ScrollWidgetModel> contexts)
		{   if(!isInitialized)
			{
				InitScroll(contexts);
			}
		...
		private void InitScroll(List<ScrollWidgetModel> contexts)
		{
			Data = new SimpleDataHelper<WidgetScrollItemModel>(this);
		...
		protected override WidgetViewsHolder CreateViewsHolder(int itemIndex)
		{
			var instance = new WidgetViewsHolder();
		...
		// *For the method's full description check the base implementation
		protected override void UpdateViewsHolder(WidgetViewsHolder newOrRecycled)
		{
			// In this callback, "newOrRecycled.ItemIndex" is guaranteed to always reflect the
			// index of item that should be represented by this views holder. You'll use this index
			// to retrieve the model from your data set
			
			WidgetScrollItemModel model = Data[newOrRecycled.ItemIndex];
			
			newOrRecycled.Widget.Init(model.Context);
		}
		...
		protected override void OnBeforeRecycleOrDisableViewsHolder(WidgetViewsHolder inRecycleBinOrVisible, int newItemIndex)
		{
		...
		protected override void OnItemIndexChangedDueInsertOrRemove(WidgetViewsHolder shiftedViewsHolder, int oldIndex, bool wasInsert, int removeOrInsertIndex)
		{
			base.OnItemIndexChangedDueInsertOrRemove(shiftedViewsHolder, oldIndex, wasInsert, removeOrInsertIndex);

			var widgetScrollItemModel = Data[shiftedViewsHolder.ItemIndex];
			shiftedViewsHolder.Widget.Init(widgetScrollItemModel.Context);
		}
		...
		public void AddItemsAt(int index, IList<WidgetScrollItemModel> items)
		{
		...
		public void SetItems(IList<WidgetScrollItemModel> items)
		{
		...
		private void RetrieveDataAndUpdate(List<ScrollWidgetModel> models)
		{
			StartCoroutine(FetchMoreItemsFromDataSourceAndUpdate(models));
		}
		...
		private IEnumerator FetchMoreItemsFromDataSourceAndUpdate(List<ScrollWidgetModel> contexts)
		{
			// Simulating data retrieving delay
			//yield return new WaitForSeconds(.5f);
			var newItems = new WidgetScrollItemModel[contexts.Count];

			// Retrieve your data here
			for (int i = 0; i < contexts.Count; ++i)
			{
				var model = new WidgetScrollItemModel()
				{ 
					Context = contexts[i]
				};
				newItems[i] = model;
		...
		private void OnDataRetrieved(WidgetScrollItemModel[] newItems)
		{
			models.Clear();
			models.AddRange(newItems);
		...
		private void UpdateData(List<ScrollWidgetModel> models)
		{
			RetrieveDataAndUpdate(models);
		}
	}
}