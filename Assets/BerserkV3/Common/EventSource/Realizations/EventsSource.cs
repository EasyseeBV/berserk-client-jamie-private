...
namespace BerserkV3.Common.EventSource
{
    public class EventsSource : IEventsSource, IEventPublisher
    {
        ...
        public virtual async Task PublishParallelAsync(object value)
        {
            ...
            {
                DefaultSharedLogger.Error($"[{GetType().Name}] Some exception caused when {nameof(PublishParallelAsync)} with registered type : {value?.GetType().FullName}. Full exception: {e}");
            }
        }
        ...
        protected virtual object DynamicInvoke(Subscriber subscriber, object value)
        {
            ...
            {
	            DefaultSharedLogger.Error($"[{GetType().Name}] Some exception caused when {nameof(PublishAsync)} event method : {subscriber.Callback?.Method.Name}, with registered type : {value?.GetType().FullName}. Full exception: {e}");
            }
