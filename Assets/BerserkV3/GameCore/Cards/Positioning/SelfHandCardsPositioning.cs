...
{
	public class SelfHandCardsPositioning : ISelfHandCardsPositioning
	{
		private readonly ISpline spline;
		private readonly IGameContext gameContext;
	
		public SelfHandCardsPositioning(
			ISplineStorage storage, 
			IGameContext gameContext)
		{
			spline = storage.Get(SplineType.HandSelf);
			this.gameContext = gameContext;
		}
		
		public IEnumerable<TargetTransform> CalculatePositions(int objectsCount)
		{
			var player = gameContext.PlayerRepository.Get(User.Id);
