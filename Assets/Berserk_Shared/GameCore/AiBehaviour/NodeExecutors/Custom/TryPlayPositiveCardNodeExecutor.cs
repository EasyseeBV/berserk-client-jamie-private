...
{
	[AiBehaviourExecutor(AiBehaviourNodeType.TryPlayPositiveCard)]
	public class TryPlayPositiveCardNodeExecutor : AbstractNodeExecutor
	{
		public override BehaviourNodeState Execute(AiNode node, IAiBehaviourExecutor behaviourExecutor)
		{
			...
			foreach (var card in possibleCards)
			{
				...
				if (effectDatas.Any(effect => effect.TargetOwner == Owner.Opponent))
					continue;
