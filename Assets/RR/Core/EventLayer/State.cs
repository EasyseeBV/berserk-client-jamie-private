...
namespace RR.Core.EventLayer
{
	...
	public class State<T, TK> : RREvent<T, TK>, IState, IEquatable<State<T, TK>>
	{
		public (T, TK) Value
		{
			get => (Data1, Data2);
		...
		public static bool operator ==(State<T, TK> state, State<T, TK> otherState)
		{
			if (state == null)
			{
				Debug.Error($"{nameof(state)} is NULL, comparing via \"==\" with {typeof(T)}:[{otherState}]");
				return false;
			}
	...
	/// </summary>
	public class State<T> : RREvent<T>, IState
	{
		...
		public static bool operator ==(State<T> state, T val)
		{
			...
			Debug.Warning($"{nameof(state)} is NULL, comparing via \"==\" with {typeof(T)}:[{val}]");
